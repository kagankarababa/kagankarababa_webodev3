...
const initialState = {
  ...
  episodes: [],
  detailedShow: null,
  watchlist: JSON.parse(localStorage.getItem('movieClubWatchlist_v2')) || [],
  isLoading: true,
  isError: false,
  query: localStorage.getItem('movieClubQuery_v2') || "spongebob",
  filters: {
    genre: "",
    language: "",
    rating: 0,
  ...
  return (
    <>
      <header className="app-header">
        <h1 className="app-title">Benim Film Kütüphanem</h1>
      </header>
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem('movieClubQuery_v2', state.query);
  }, [state.query]);

  useEffect(() => {
    localStorage.setItem('movieClubWatchlist_v2', JSON.stringify(state.watchlist));
  }, [state.watchlist]);
